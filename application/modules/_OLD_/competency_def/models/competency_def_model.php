...
class Competency_def_model extends CI_Model
{
    ...
    public function create_level($title = FALSE, $additional_data = array())
    {
        ...
        // bail if the group name already exists
        $existing_competency_level = $this->db->get_where($this->tables['competency_level'], array('title' => $title))->num_rows();
        if($existing_competency_level !== 0)
        {
            $this->set_error('competency_level_already_exists');
            return FALSE;
        }
        ...
        // report success
        $this->set_message('competency_level_creation_successful');
        // return the brand new id
        return $id;
    }

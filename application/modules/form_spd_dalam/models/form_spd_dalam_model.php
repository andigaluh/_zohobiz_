...
class Form_spd_dalam_model extends CI_Model
{
     /**
     ...
     * form_spd_dalam
     *
     * @return object form_spd_dalam
     * @author Deni
     **/
    public function form_spd_dalam()
    {
        $this->trigger_events('form_spd_dalam');
        ...
        else
        {
            ...
            $this->db->join('users', 'users_spd_dalam.user_id = users.id', 'left');
    ...
    {
         $this->trigger_events('form_spd_dalam_input');
    ...
    public function create_($user_id = FALSE, $additional_data = array())
    {
        ...
        if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['users_spd_dalam'], $additional_data), $data);

        $this->trigger_events('extra_group_set');

        // insert the new form_spd_dalam
        $this->db->insert($this->tables['users_spd_dalam'], $data);
    ...
    {
        $this->trigger_events('pre_update_frm_spd_dalam');

        $form_spd_dalam = $this->form_spd_dalam($id)->row();

        $this->db->trans_begin();

        // Filter the data passed
        $data = $this->_filter_data($this->tables['users_spd_dalam'], $data);

        $this->trigger_events('extra_where');
        $this->db->update($this->tables['users_spd_dalam'], $data, array('id' => $form_spd_dalam->id));

        if ($this->db->trans_status() === FALSE)
        {
            $this->db->trans_rollback();

            $this->trigger_events(array('post_update_form_spd_dalam', 'post_update_form_spd_dalam_unsuccessful'));
        ...
        $this->trigger_events(array('post_update_form_spd_dalam', 'post_update_frm_cuti_unsuccessful'));

...
class position_group extends MX_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('position_group_model','position_group_model');
    }

    var $title = 'Position Group';
    var $limit = 100000;
    var $controller_name = 'position_group';
    var $model_name = 'position_group_model';
    var $id_table = 'id';
    var $list_view = 'position_group/index';
    ...
    {
        $list = $this->position_group_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $val) {
            $no++;
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->position_group_model->count_all(),
                        "recordsFiltered" => $this->position_group_model->count_filtered(),
                        "data" => $data,
                );
    ...
    public function ajax_add()
    {
        ...
        $insert = $this->position_group_model->save($data);
    ...
    public function ajax_delete($id)
    {
        ...
        $this->position_group_model->update(array('id' => $id), $data);
        echo json_encode(array("status" => TRUE));
    }

    public function ajax_edit($id)
    {
        $data = $this->position_group_model->get_by_id($id);
    ...
    private function _validate()
    {
        ...
        if($this->input->post('abbr') == '')
        {
            $data['inputerror'][] = 'abbr';
            $data['error_string'][] = 'Abbr is required';
            $data['status'] = FALSE;
        }

        if($this->input->post('level_order') == '')
        {
            $data['inputerror'][] = 'level_order';
            $data['error_string'][] = 'Level order is required';
            $data['status'] = FALSE;
        }
    ...
    function _render_page($view, $data=null, $render=false)
    {
        ...
        if ( ! $render)
        {
            ...
            if (in_array($view, array('position_group/index')))
            {
                ...
                $this->template->add_js('position_group.js');
